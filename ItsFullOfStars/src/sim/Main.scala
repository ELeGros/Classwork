...
object Main {
  ...
  def main(args: Array[String]): Unit = {
    val parts = for(i <- 1 to 5000) yield(new Particle(Point3D(util.Random.nextGaussian()*10,util.Random.nextGaussian()*10,util.Random.nextGaussian()*10), Vect3D(0,0,0),util.Random.nextDouble(), util.Random.nextDouble()))
